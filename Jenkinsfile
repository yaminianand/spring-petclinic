pipeline{
    ...
    stages{
		...
		sh "sudo docker run -d -p 8091:8080 anjurose/petclinic"
                ...
                sh "sudo puppet resource cron puppet-agent ensure=present user=root minute=1 command='/opt/puppetlabs/bin/puppet agent --onetime --no-daemonize --splay --splaylimit 60'"   
            }
        }
    }
}
